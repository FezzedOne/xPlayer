...
function collisionAbove(posToCheck, maxDist)
    ...
    for i = 1, math.floor(maxDist or 1), 1 do
        ...
        for j = -2, 2, 1 do
            ...
            if
                world.pointCollision(adjPos, { "Block", "Dynamic", "Slippery" })
                or world.pointTileCollision(adjPos, { "Block", "Dynamic", "Slippery" })
            then
                return i
            end
        end
...
function PathFinder:canPathfind()
    ...
    return mcontroller.onGround()
        or not baseParameters.gravityEnabled
        or self.options.tryFlying
        or self.options.alwaysFly
end
...
function PathFinder:update(targetPosition)
    ...
    if not self.hasPath then
        ...
        if self.options.alwaysFly then
            ...
            if
                self.options.tryFlying
                and (
                    mcontroller.zeroG()
                    or mcontroller.liquidMovement()
                    or (not groundBelow(targetPosition, maxJumpHeight))
                    or findProgress == false
                )
            then
...
function PathMover:new(options)
    ...
    if pathMovementParams and pathMovementParams.airJumpProfile then
        newPather.canMultiJump = not not pathMovementParams.airJumpProfile.multiJump
    end
    ...
    if config then
        ...
        newPather.canOpenDoors = false
        newPather.forceWalkingBackwards = false
    end
...
function PathMover:openDoors()
    ...
    if world.rectTileCollision(bounds, { "Dynamic" }) then
        ...
        local closedDoorIds = world.entityQuery(
            rect.ll(bounds),
            rect.ur(bounds),
            { includedTypes = { "object" }, callScript = "hasCapability", callScriptArgs = { "closedDoor" } }
        )
...
function PathMover:moveJump()
    ...
    if world.magnitude(mcontroller.position(), self.edge.source.position) < 1.0 then
        ...
        if mcontroller.liquidMovement() or self.jumpTimer <= 0 then
            ...
            if self.options.pathOptions and self.options.pathOptions.glideAdjustment then
                yJumpVelocity = yJumpVelocity * 0.85
            end
...
function PathMover:moveLand()
    if
        (mcontroller.onGround() or (mcontroller.liquidMovement() and math.abs(self.delta[2]) < 1))
        and math.abs(self.delta[1]) < 1
    then
        self:advancePath()
    ...
    if self.edge and self.edge.action == "Walk" then
        ...
        for i = 1, 5 do
            ...
            if edge and edge.action == "Walk" then
                if
                    edge.target.velocity
                    and math.abs(edge.target.velocity[1]) > mcontroller.baseParameters().walkSpeed
                then
                    run = true
                end
            else
